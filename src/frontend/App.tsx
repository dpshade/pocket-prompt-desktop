...
} from "@/frontend/components/search/SearchBar";
...
import { TitleBar } from "@/frontend/components/shared/TitleBar";
...
import {
  useSyncModeStatus,
  validateDirectoryOnStartup,
} from "@/frontend/hooks/useSyncMode";
...
function App() {
  ...
  const {
    password,
    setPassword,
    hasPassword: hasPasswordFromContext,
    setWalletAddress,
    isLoadingPassword,
  } = usePassword();

  // When encryption is disabled, treat as always having password
  const hasPassword = FEATURE_FLAGS.ENCRYPTION_ENABLED
    ? hasPasswordFromContext
    : true;
  ...
  const [sampleEncryptedData, setSampleEncryptedData] =
    useState<EncryptedData | null>(null);
  ...
  const applyProtocolLinkParamsRef = useRef<
    (params: ProtocolLinkParams, source?: string) => void
  >(() => {});
  ...
  const actuallyApplyParams = useCallback(
    (params: ProtocolLinkParams, source = "unknown") => {
      console.log(
        "[App] Actually applying protocol link params:",
        params,
        "source:",
        source,
      );
      ...
      deepLinkStateRef.current.sources[source] =
        (deepLinkStateRef.current.sources[source] || 0) + 1;
      ...
      try {
        switch (params.type) {
          ...
          case "search":
            ...
            if (params.query) {
              ...
              console.log(
                "[App] Before setSearchQuery - current query:",
                searchQuery,
              );
            ...
            if (params.expression) {
              console.log(
                "[App] Parsing and applying boolean expression:",
                params.expression,
              );
              const expression = urlParamToExpression(params.expression);
              if (expression) {
                console.log(
                  "[App] Successfully parsed expression, applying:",
                  expression,
                );
                console.log(
                  "[App] Before setBooleanExpression - current expression:",
                  booleanExpression,
                );
                ...
                console.error(
                  "[App] Failed to parse boolean expression from param:",
                  params.expression,
                );
        ...
        console.error(
          "[App] Error applying deep link params:",
          error,
          "params:",
          params,
        );
        ...
        if (
          ...
          deepLinkRetryRef.current.maxAttempts
        ) {
          deepLinkRetryRef.current.attempts++;
          ...
          setTimeout(() => {
            if (deepLinkRetryRef.current.params) {
              ...
              actuallyApplyParams(
                deepLinkRetryRef.current.params!,
                `${source}-retry-${deepLinkRetryRef.current.attempts}`,
              );
          ...
          console.error(
            "[App] Max retry attempts reached for deep link:",
            params,
          );
          deepLinkRetryRef.current.params = null;
        }
  ...
  useEffect(() => {
    applyProtocolLinkParamsRef.current = (
      params: ProtocolLinkParams,
      source = "unknown",
    ) => {
      console.log(
        "[App] applyProtocolLinkParams called:",
        params,
        "source:",
        source,
      );
      ...
      if (!connected || !hasPassword || prompts.length === 0) {
        console.log(
          "[App] App state not ready, storing deep link for later:",
          appState,
        );
    ...
    if (
      ...
      console.log(
        "[App] App now ready, applying pending deep link:",
        pendingDeepLinkRef.current,
      );
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      updateDeepLink({
        ...
        expr:
          booleanExpression && !activeSavedSearch
            ? expressionToString(booleanExpression)
            : undefined,
        collection: activeSavedSearch?.id,
        // Don't include prompt param if we're using txid
        prompt:
          !txidParam && viewDialogOpen && selectedPrompt
            ? selectedPrompt.id
            : undefined,
  ...
  useEffect(() => {
    if (hasPassword) {
      console.log(
        "[App] Password available, closing any open password dialogs",
      );
    ...
    const checkForEncryptedPrompts = async () => {
      ...
      if (address) {
        ...
        try {
          ...
          if (storedPassword) {
            console.log(
              "[App] Password found in localStorage, skipping dialog",
            );
            passwordCheckDone.current = true;
            return;
          }
      ...
      if (hasPassword) {
        console.log(
          "[App] Password already loaded in state, marking check done",
        );
        passwordCheckDone.current = true;
        return;
      }
      ...
      console.log(
        "[App] No password found anywhere, checking for encrypted prompts...",
      );
      passwordCheckDone.current = true;
  ...
  useEffect(() => {
    ...
    if (identity.connected) {
      ...
      if (syncStatus.mode === "attached-directory") {
        console.log(
          "[DEBUG] Validating directory accessibility before loading prompts...",
        );
        validateDirectoryOnStartup().then((isValid) => {
          if (isValid) {
            console.log(
              "[DEBUG] Directory validation passed, calling loadPrompts()...",
            );
            loadPrompts();
          } else {
            console.error(
              "[DEBUG] Directory validation failed, not loading prompts",
            );
          }
        });
      } else {
        console.log(
          "[DEBUG] Not in attached directory mode, calling loadPrompts() directly...",
        );
        loadPrompts();
      }
    }
  ...
  const duplicateIds = useMemo(() => {
    ...
    return new Set(
      findDuplicates(prompts).flatMap((group) =>
        group.prompts.map((p) => p.id),
      ),
    );
    ...
    const finalResult = prompts
      ...
      .sort((a, b) => {
        ...
        if (effectiveSearchQuery && searchScoreMap.size > 0) {
          return (
            (searchScoreMap.get(b.id) || 0) - (searchScoreMap.get(a.id) || 0)
          );
        }
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") {
        ...
        const isSearchInput =
          ...
          target.getAttribute("placeholder")?.includes("Search");
        if (isSearchInput) {
          event.preventDefault();
  ...
  const handleMouseEnterItem = useCallback(
    (index: number) => {
      ...
      if (
        isKeyboardMode &&
        index !== selectedIndex &&
        mouseMovedSinceKeyboard.current
      ) {
        setIsKeyboardMode(false);
      }
    },
  ...
  const handleSave = async (data: Partial<Prompt>) => {
    if (editingPrompt) {
      ...
      return await addPrompt(
        data as Omit<Prompt, "id" | "createdAt" | "updatedAt">,
        password || undefined,
      );
    }
  };
  ...
  const handleBatchImport = async (selectedPrompts: FileImportResult[]) => {
    ...
    for (const result of selectedPrompts) {
      ...
      try {
        ...
        errors.push(
          `${result.fileName}: ${err instanceof Error ? err.message : "Unknown error"}`,
        );
      }
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <div className="text-center space-y-6 max-w-md">
          ...
          <div className="flex items-center justify-center">
            <div
              className="animate-spin inline-block w-8 h-8 border-4 border-primary/30 border-t-primary rounded-full"
              role="status"
            >
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <div
                    className={`hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-medium ${directorySyncing ? "bg-amber-500/10 text-amber-600 dark:text-amber-400" : "bg-green-500/10 text-green-600 dark:text-green-400"}`}
                  >
                    <FolderSync
                      className={`h-3.5 w-3.5 ${directorySyncing ? "animate-spin" : ""}`}
                    />
      ...
      {/* Mobile Fixed Header - Logo at Top */}
      <header className="sm:hidden fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-xl border-b border-border/30">
        ...
        <div className="w-full max-w-2xl">
          ...
          <div className="flex flex-col w-full">
            ...
            <div className="hidden sm:flex items-center justify-center gap-3 mb-8">
              ...
              />
              <h1 className="font-bold text-foreground text-4xl tracking-tight">
                Pocket Prompt
              </h1>
            </div>
            ...
            <div
              ...
              className="hidden sm:block rounded-2xl shadow-soft-lg overflow-hidden ring-1 ring-primary/10"
            >
              <SearchBar
                ref={searchBarRef}
              ...
              {filteredPrompts.length > 0 && (
                <div
                  className="bg-[#faf8fc] dark:bg-card border-t border-border/30"
                ...
                >
                  <div className="max-h-[300px] overflow-y-auto">
                    {filteredPrompts.map((prompt, index) => (
                      <PromptListItem
                        ...
                        data-selected={
                          isKeyboardMode && index === selectedIndex
                        }
                      />
                    ))}
                  </div>
                  <div className="px-3 py-1.5 text-center text-[11px] text-muted-foreground border-t border-border/20 bg-muted/20">
                    ...
                    {(() => {
                      const totalActive = prompts.filter(
                        (p) => !p.isArchived,
                      ).length;
                      return filteredPrompts.length !== totalActive &&
                        !showArchived
                        ? ` of ${totalActive}`
                        : "";
                    })()}
            ...
            <div className="sm:hidden mt-4">
              {filteredPrompts.length > 0 && (
                <>
                  <div
                    className="bg-card rounded-3xl overflow-hidden shadow-soft-lg"
                    data-keyboard-mode={isKeyboardMode}
                  >
                    {filteredPrompts.map((prompt, index) => (
                      <PromptListItem
      ...
      <PromptDialog
        ...
        onArchive={() =>
          archivePrompt(selectedPrompt!.id, password || undefined)
        }
        onShowVersions={() => {
          setViewDialogOpen(false);
