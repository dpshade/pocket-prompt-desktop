...
export const SearchBar = forwardRef<SearchBarHandle, SearchBarProps>(
  (
    ...
    return (
      <>
        <div
          className={`bg-gradient-to-b from-white to-[#faf8fc] dark:bg-card dark:from-transparent dark:to-transparent relative px-4 pt-3 sm:px-3 sm:pt-2.5 space-y-2 ${connectedBottom ? "pb-0" : "pb-3 sm:pb-2.5 rounded-2xl shadow-soft ring-1 ring-primary/10"}`}
        >
          <div className="relative flex-1">
            ...
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-0.5">
              ...
              <Button
                variant={
                  booleanExpression || showBooleanBuilder ? "default" : "ghost"
                }
                size="sm"
              ...
              {showTagSuggestions && (
                <div className="flex flex-wrap gap-2.5 sm:gap-2">
                  {allTags.length > 0 ? (
                        ...
                        <Badge
                          ...
                          onClick={() => {
                            ...
                            if (updatedTags.length > 0) {
                              const tagsExpression = updatedTags
                                .map((t) => `"${t}"`)
                                .join(" AND ");
                              setExpressionText(tagsExpression);
                              try {
                                const parsed =
                                  parseBooleanExpression(tagsExpression);
                    ...
                    <div className="text-[13px] sm:text-xs text-muted-foreground">
                      No tags yet. Add tags to your prompts!
                    </div>
                  )}
                </div>
              )}
